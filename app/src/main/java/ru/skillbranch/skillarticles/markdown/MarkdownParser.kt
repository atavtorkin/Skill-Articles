...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        ...
        return prepare(findElements(string)).fold(StringBuilder()) { acc, element ->
            acc.append(element.text)
        }.toString()
    }

    private fun prepare(list: List<Element>): List<Element> {
        return list.fold(mutableListOf()) { acc, el ->
            acc.also {
                if (el.elements.isNotEmpty())
                    acc.addAll(prepare(el.elements))
                else acc.also { acc.add(el) }
            }
        }
    }
